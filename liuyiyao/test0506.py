...
class ResNeXt3D(nn.Module):
    ...
    def __init__(self, block, layers, shortcut_type='B', cardinality=32, num_classes=400):
        ...
        self.layer2 = self._make_layer(block, 128, layers[1], shortcut_type, cardinality, stride=( 2, 2))
...
class PPM(nn.Module):
    ...
    def forward(self, x):
        ...
        xi = x[:, :, :, :, None, None]
        print("PPM:xi.shape:", xi.shape)
        xj = x[:, :, None, None, :, :]
        print("PPM:xj.shape:", xj.shape)
...
class PixelCL(nn.Module):
    ...
    def forward(self, x, return_positive_pairs = False):
        ...
        print("proj_coors_one[:, None].shape:", proj_coors_one[:, None].shape)
        print("proj_coors_two[None, :].shape:", proj_coors_two[None, :].shape)
        ...
        print("proj_coors_one_expanded.shape:",proj_coors_one_expanded.shape)
        print("proj_coors_two_expanded.shape:",proj_coors_two_expanded.shape)
...
if __name__ == '__main__':
    ...
    net =  ResNeXt3D(ResNeXtBottleneck, [3, 4, 6, 3], num_classes=20)
    train = PixelCL(net,(128,256))
    train(image)